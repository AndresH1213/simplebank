...
func TestGetTransferAPI(t *testing.T) {
	...
	request, err := http.NewRequest(http.MethodGet, fmt.Sprintf("/transfers/%d", transfer.ID), nil)
	addAuthorization(t, request, server.tokenMaker, authorizationType, user.Username, time.Minute)
...
func TestTransferAPI(t *testing.T) {
	...
	account1 := randAccount(user1.Username)
	account2 := randAccount(user2.Username)
	account3 := randAccount(user3.Username)
	...
	testCases := []struct {
		...
		},
		{
			...
			setupAuth: func(t *testing.T, request *http.Request, tokenMaker token.Maker) {
				addAuthorization(t, request, tokenMaker, authorizationType, user3.Username, time.Minute)
			},
			buildStubs: func(store *mockdb.MockStore) {
